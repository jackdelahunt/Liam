...
void SymbolTable::add_local_type(Token token, TypeInfo *type_info) {
    if (this->local_generic_type_table.count(token.string) > 0)
    {
        TypeCheckerError::make(this->current_file->file_data->path.string())
    ...
    this->local_generic_type_table[token.string] = type_info;
}
...
void TypeChecker::type_check_fn_symbol(FnStatement *statement) {
    this->add_function(
        this->current_file, statement->identifier, new FnTypeInfo(statement->flag_mask, NULL, NULL, {}, {})
    );
}
...
void TypeChecker::type_check_struct_symbol(StructStatement *statement) {
    ...
    if (!failed)
    {
        ...
        TypeCheckerError::make(current_file->file_data->path.string())
...
void TypeChecker::type_check_enum_symbol(EnumStatement *statement) {
    ...
    // add it to the table and leave its type info blank until we type check it
    this->add_type(
        this->current_file, statement->identifier, new EnumTypeInfo(std::vector<EnumMember>(), statement->flag_mask)
    );
}
            ...
            else
            {
                if (!type_match(fn_type_info->return_type, rt->expression->type_info))
                {
                    ErrorReporter::report_type_checker_error(
                        current_file->file_data->path.string(), rt->expression, NULL, statement->return_type, NULL, {},
...
void TypeChecker::type_check_let_statement(LetStatement *statement, SymbolTable *symbol_table) {
    ...
    if (statement->type)
    {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), statement->rhs, NULL, statement->type, NULL, {},
...
void TypeChecker::type_check_assigment_statement(AssigmentStatement *statement, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), statement->lhs, statement->assigned_to->expression, NULL, NULL, {},
...
void TypeChecker::type_check_binary_expression(BinaryExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression->left, expression->right, NULL, NULL, {},
...
void TypeChecker::type_check_number_literal_expression(NumberLiteralExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Problem parsing number literal"
        );
        return;
    }
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Cannot use decimal point on non float types"
        );
        return;
    }
    ...
    case NumberType::UNSIGNED: {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Unsigned number cannot be negative"
            );
            return;
        }
    ...
    case NumberType::FLOAT: {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), expression, NULL, NULL, NULL, {},
...
void TypeChecker::type_check_unary_expression(UnaryExpression *expression, SymbolTable *symbol_table) {
    ...
    if (expression->op.type == TokenType::TOKEN_STAR)
    {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Cannot dereference non-pointer value"
            );
            return;
        }
...
void TypeChecker::type_check_subscript_expression(SubscriptExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression->lhs, NULL, NULL, NULL, {},
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression->expression, NULL, NULL, NULL, {},
...
void TypeChecker::type_check_call_expression(CallExpression *expression, SymbolTable *symbol_table) {
    ...
    ErrorReporter::report_type_checker_error(
        current_file->file_data->path.string(), callee_expression, NULL, NULL, NULL, {}, "Can only call functions"
    );
}
...
void TypeChecker::type_check_fn_call_expression(CallExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), callee_expression, NULL, NULL, NULL, {},
            fmt::format(
    ...
    for (i32 i = 0; i < fn_type_info->args.size(); i++)
    {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), callee_expression, expression->args.at(i), NULL, NULL, {},
...
void TypeChecker::type_check_get_expression(GetExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            this->current_file->file_data->path, expression->lhs, NULL, NULL, NULL, {}, "Cannot derive member from non struct type"
        );
        return;
    }
    ...
    for (auto stmt : expression->body->statements)
    {
        if (stmt->statement_type == StatementType::STATEMENT_RETURN)
        {
            ...
            if (expression->return_type->type_info->type == TypeInfoType::VOID)
            {
                ...
                {
                    ErrorReporter::report_type_checker_error(
                        current_file->file_data->path.string(), rt->expression, NULL, NULL, NULL, {},
            ...
            else
            {
                ...
                {
                    ErrorReporter::report_type_checker_error(
                        current_file->file_data->path.string(), rt->expression, NULL, expression->return_type, NULL, {},
                        "Mismatch types in function, return types do not match"
                    );
                    return;
                }
...
void TypeChecker::type_check_slice_literal_expression(SliceLiteralExpression *expression, SymbolTable *symbol_table) {
    ...
    for (auto passed_expression : expression->expressions)
    {
        ...
        {
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), passed_expression, NULL, expression->type_expression, NULL, {},
...
void TypeChecker::type_check_instantiate_expression(InstantiateExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression, expression->expression, NULL, NULL, {},
...
void TypeChecker::type_check_enum_instance_expression(EnumInstanceExpression *expression, SymbolTable *symbol_table) {
    ...
    {
        ErrorReporter::report_type_checker_error(
            this->current_file->file_data->path.string(), expression, NULL, NULL, NULL, {},
    ...
    for (i64 i = 0; i < actual_arg_amount_given; i++)
    {
        ...
        {
            ErrorReporter::report_type_checker_error(
                this->current_file->file_data->path.string(), expression, argument, NULL, NULL, {},
...
void TypeChecker::type_check_struct_instance_expression(
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Unrecognised type in new expression"
        );
        return;
    }
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), expression, NULL, NULL, NULL, {}, "Can only use struct types in new expression"
        );
        return;
    }
    ...
    for (i32 i = 0; i < calling_args_type_infos.size(); i++)
    {
        ...
        if (expression_member != member)
        {
            ...
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), expr, NULL, NULL, NULL, {}, "Incorrect name specifier in new expression"
            );
            return;
        }
        ...
        if (!type_match(resolved_member_type, expression_type))
        {
            auto [name, expr] = expression->named_expressions.at(i);
            ErrorReporter::report_type_checker_error(
                current_file->file_data->path.string(), expression, expr, NULL, NULL, {}, "Mismatched types in new expression"
            );
            return;
        }
    }
...
void TypeChecker::type_check_specified_generics_type_expression(
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), NULL, NULL, NULL, type_expression->struct_type, {},
...
void TypeChecker::type_check_identifier_type_expression(
    ...
    {
        ErrorReporter::report_type_checker_error(
            current_file->file_data->path.string(), NULL, NULL, type_expression, NULL, {}, "Unrecognised type in type expression"
        );
        return;
    }
