...
const char *TokenTypeStrings[55] = {"int Literal", "string Literal",
                                    "identifier",  "let",
                                    "fn",          "(",
                                    ")",           "{",
                                    "}",           "+",
...
void Lexer::lex() {
    ...
    for (; current < chars->size(); next_char())
    {
        ...
        default:
            ...
            if (word == "test")
            {
                tokens.emplace_back(Token(TokenType::TOKEN_TEST, word, current_line, word_start));
                continue;
            }
            ...
            if (word == "make")
            {
                tokens.emplace_back(Token(TokenType::TOKEN_MAKE, word, current_line, word_start));
                continue;
            }
